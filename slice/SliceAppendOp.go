...
func main() {
	...
	nums := []int{1}
	fmt.Printf("Length: %d, Capacity: %d, Address: %p\n", len(nums), cap(nums), &nums) // Length: 1, Capacity: 1

	nums = append(nums, 2)
	fmt.Printf("Length: %d, Capacity: %d, Address: %p\n", len(nums), cap(nums), &nums) // Length: 2, Capacity: 2

	nums = append(nums, 3)
	fmt.Printf("Length: %d, Capacity: %d, Address: %p\n", len(nums), cap(nums), &nums) // Length: 3, Capacity: 4
	...
	fmt.Printf("Length: %d, Capacity: %d, Address: %p\n", len(nums), cap(nums), &nums) // Length: 5, Capacity: 8

	letters := []string{"a", "b", "c", "d", "e", "f"}

	fmt.Println("*Capacity*", cap(letters))

	fmt.Println("Letters :", letters)

	subSlice := append(letters[:2], "x", "z")

	fmt.Println(letters)

	fmt.Println("Subslice :", subSlice)
	fmt.Printf("Length: %d, Capacity: %d, Address: %p\n", len(subSlice), cap(subSlice), &subSlice) // Length: 5, Capacity: 8

	fmt.Println("subSlice[5] :", subSlice[3])

	fmt.Println("subslice[2:5] :", subSlice[2:5])
